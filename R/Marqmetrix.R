...
scan.txt.marqmetrix.header <- function(file) {
    ...
    software.version <- hdr[min(which(match(hdr, "Software Version:") == TRUE))+1]
    spectrometer.sn <- hdr[min(which(match(hdr, "Spectrometer SN:") == TRUE))+1]
    ...
    sequence.number <- as.numeric(
    			    hdr[min(which(match(hdr, "Sequence Number:") == TRUE))+1])
    processing <- hdr[min(which(match(hdr, "Processing:") == TRUE))+1]
    background <- hdr[min(which(match(hdr, "Background:") == TRUE))+1]
    spectral.points <- as.numeric(
     			    hdr[min(which(match(hdr, "Spectral Points:") == TRUE))+1])
